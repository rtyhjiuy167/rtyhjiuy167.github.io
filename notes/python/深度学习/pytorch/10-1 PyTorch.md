...
print(dates)  # [datetime.datetime(2016, 1, 1, 0, 0) ,..., datetime.datetime(2016, 12, 31, 0, 0)]
...
my_nn = torch.nn.Sequential(
    ...
    torch.nn.Sigmoid(),  # 激活函数 这里用 Sigmoid()
    torch.nn.Linear(hidden_size, output_size),
)
cost = torch.nn.MSELoss(reduction='mean')  # 损失函数 直接调包
optimizer = torch.optim.Adam(my_nn.parameters(), lr=0.001)  # 优化器 这里用 Adam , SGD是随机梯度下降
...
for i in range(1000):
    ...
    for start in range(0, len(input_features), batch_size):
        ...
        prediction = my_nn(x)  # 将x传入到已经定义好的神经网络，进行前向传播，得到预测值
        loss = cost(prediction, y.view(y.size()[0], 1))
        optimizer.zero_grad()
...
plt.plot(dates, predict.reshape(-1), label='prediction')
plt.legend()
